<?php
    ...
    $variant = data_get($field, 'variant', false);

    if ( !is_array($field['value']) ) {
        $field[ 'value' ] = !blank($field[ 'value' ]) ? [ $field[ 'value' ] ] : [];
    }

    if($multiple && $relation && $crud && $crud->entry && !$variant) {
        $values = $crud->entry->{$relation}()->get();
        if(!blank($values)) {
            $field['value'] = $values;
        }
    } else if($multiple && $relation && $entry && $variant) {
                ...
                <div class="media-preview-wrapper">
                    <div class="media-preview">
                        ...
                        <a id="{{ $field['name'] }}_{{ $media->id }}_clear_button" href="#"
                           class="btn btn-xs media-clear-button" title="Clear file" data-media-id="{{ $media->id }}" data-field-name="{{ $field['name'] }}"><i
    ...
    @push('crud_fields_scripts')
        ...
        <script>
          ...
          $(document).on('click', ".media-clear-button", function ( e ) {
            ...
            $("<input type='hidden' name='clear_"+ fieldName +"{{$field_name_suffix}}' value='" + mediaId + "'>").insertAfter("#{{ $field['name'] }}_file_input");
          });
        </script>
    @endpush
@endif
