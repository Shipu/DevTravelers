    ...
    <div class="row">
        <div class="col-sm-6" style="margin-bottom: 20px;">
            ...
            @if(isset($entry) && count($entry->values) > 0)
                <img id="mainImage" src="{{ url(config('filesystems.disks.'.(isset($field['disk']) ? $field['disk'] : 'media.url')).'/'.$entry->values->first()->value) }}">
            @else
                <img id="mainImage" src="{{ old($field['name']) ? old($field['name']) : (isset($field['value']) ? $field['value'] : (isset($field['default']) && Storage::disk('media')->has($field['default'])) ? url(config('filesystems.disks.'.(isset($field['disk']) ? $field['disk'] : 'media.url')).'/'.$field['default']) : '')  }}">
            @endif
        </div>
